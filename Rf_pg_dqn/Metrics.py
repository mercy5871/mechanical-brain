...
class Metrics(object):
    ...
    def score_on_topk(self, actual, predict, groups=[0], k=50, show_rank=True, show_score=False, plot_move_window=1):
        """
        ...
        ])).sort_values(["group", "predict"], ascending=[True, False]).reset_index(drop=True)
        gsize = df.groupby("group")["predict"].count()
        df["rank"] = df.groupby("group")["actual"].rank(ascending=False, pct=True)
        ...
        if show_rank:
            ...
            plt.plot(rdf["group"], rdf["rank"], 'o', color="0.4", alpha=0.3, markersize=3)
        ...
        if show_score:
            ...
            plt.plot(rdf["group"], rdf["actual"], 'o', color='0.4', alpha=0.3, markersize=3)
            plt.legend(loc="upper left", fontsize=self.fontsize)
            self.show()
        return rdf

    def plot_trade_summary(self, indices, value, benchmark, log_scale=False, account_label="value", benchmark_label="benchmark", frequency='D', Rf=0.03):
        '''
        展示净值曲线变化
    ...
    def plot_trade_log(self, trade_log, log_scale=False, window_size=1, bins=256, account_label="value", benchmark_label="benchmark", frequency='D', Rf=0.03):
        ...
        plt.subplot(323)
        sns.distplot(hold_df["period"], bins=bins, label=f'period = {round(hold_df["period"].mean(), 2)}')
        ...
        plt.legend(loc="upper right")
        plt.subplot(325)
        sns.distplot(hold_df["max_drawdown"], bins=bins, label=f'max_drawdown = {round(hold_df["max_drawdown"].mean()*100, 2)}%')
        plt.legend(loc="upper right")
        plt.subplot(326)
