...
def _back_test(
    ...
    current_k = 0
    # 0: "try_count", 1: "try_success_count"
    try_count = np.zeros(n_cycle, dtype=np.int32)
...
def back_test(
    ...
    trade_filter = np.ones(np.array(col_buy_flag).shape[0], dtype=np.bool) \
        if trade_filter is None else np.array(trade_filter, dtype=np.bool)
    ...
    value_all, buy_sell_count, hold_log, hold_indices, try_count = _back_test(
        ...
        trade_filter=np.array(base_df["trade_filter"])
    )
    t_ed = time.time()
    if verbose == True:
